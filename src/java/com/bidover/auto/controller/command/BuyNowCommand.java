...
public class BuyNowCommand implements ICommand {
    ...
    public void execute() throws ServletException, IOException {
        ...
        try {
            ...
            if (!utl.isInt(request.getParameter("lotQnt"))) result = "{'status':0, 'message':'INCORRECT NUMBER FORMAT!'}";
            else if (Integer.parseInt(request.getParameter("lotQnt")) < 1) result = "{'status':0, 'message':'QNT LESS THAN 1!'}";
            else if (qnt_items_available < Integer.parseInt(request.getParameter("lotQnt"))) result = "{'status':0, 'message':'QNT MORE THAN AVAILABLE!'}";
            else if (userBalance < fee) result = "{'status':0, 'message':'You have not sufficient funds on the account! Your balance is: "+userBalance+" USD.'}";
            else {
                // INSERT BID TO BIDHISTORY FILE
                FileIOSet fio = new FileIOSet();
                String fileName = lotId+"/bidhistory.dat";
                String dataString = System.currentTimeMillis()+";"+userId+";"+lotFixedPrice;
